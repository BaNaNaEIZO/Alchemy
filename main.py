...
class AddressTable(Base):
    __tablename__ = "adresses"  # noqa
    id: Mapped[int] = mapped_column(primary_key=True)
    category: Mapped[str] = mapped_column()
    category_ID: Mapped[int] = mapped_column(ForeignKey("furniture.category_ID"))
...
with Session(engine) as f_session:
    with f_session.begin():
        for row in df.to_dict(orient='records'):
            i += 1
            furniture = FurnitureTable(id=i, item_id=row["item_id"], name=row["name"], price=row["price"],
    ...
    with f_session.begin():
        res = f_session.execute(select(FurnitureTable).where(FurnitureTable.id == 8))
        furniture2 = res.scalar()
...
with Session(engine) as a_session:
    with a_session.begin():
        for row in df1.to_dict(orient='records'):
            i += 1
            address = AddressTable(id=i, category=row["category"], category_ID=row["category_ID"])
            a_session.add(address)
    with a_session.begin():
        res = a_session.execute(select(AddressTable).where(AddressTable.id == 2))
        address1 = res.scalar()
